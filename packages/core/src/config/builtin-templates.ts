...
export const BUILTIN_TEMPLATES: Record<string, Template> = {
  tsc: {
    description: 'Use when running TypeScript compiler (tsc) - returns type errors and error summary',
  ...
  'maven-build': {
    description: 'Use when building with Maven (mvn compile/install/package) - returns build errors and summary',
    include_regex: '(ERROR|FAILURE|BUILD FAILURE)',
    tail_paragraphs: 1,
  },

  'maven-test': {
    description: 'Use when running Maven tests (mvn test) - returns test failures and test summary',
