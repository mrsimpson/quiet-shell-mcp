 ...
 */
export function loadConfig(configPath: string, logger?: Logger): TemplateConfig {
  try {
    ...
    logger?.info(`Loaded ${Object.keys(templates).length} template(s) from config`);
...
export function validateTemplate(template: unknown): Template {
  ...
  if (typeof t['tail_paragraphs'] !== 'number' || t['tail_paragraphs'] < 0) {
    throw new Error('Template must have "tail_paragraphs" as non-negative number');
  }
